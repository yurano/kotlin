...
internal fun configureStdlibDefaultDependency(project: Project) = with(project) {
    ...
    project.kotlinExtension.sourceSets.all { kotlinSourceSet ->
        scopesToHandleConfigurations.forEach { scope ->
            ...
            if (scopeConfiguration.state == Configuration.State.RESOLVED) {
                logger.info("Could not add the Kotlin stdlib dependency to $scopeConfiguration as it is already resolved")
                return@forEach
            }
            ...
            project.tryWithDependenciesIfUnresolved(scopeConfiguration) { dependencies ->
                ...
                addStdlibDependency(project, kotlinSourceSet, dependencies)
            }
        }
    }
}

private fun addStdlibDependency(
    project: Project,
...
private fun Project.tryWithDependenciesIfUnresolved(configuration: Configuration, action: (DependencySet) -> Unit) {
    ...
    try {
        ...
        logger.info("Could not setup Kotlin-specific dependencies for $configuration")
    }
}